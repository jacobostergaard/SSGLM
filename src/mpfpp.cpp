  ...
  */
   ...
   */
    ...
    // if X trajectory is given, no need to filter => no need for the fixed lag smoother
        ...
        if(x.n_cols > 1){
          x = x.t();        // make sure x is interpreted as a column vector.
        }
    ...
    // Declaring overall parameters and containers
        int lag_b  = Sb.n_rows;                           // length of the bursting kernel
        int lag_r  = Sr.n_rows;                           // length of the resting kernel
        int lag_s;                                        // lag_s used at time n, only needed when n < lag_s_in (everything is replaced then!)
        ...
        vec H_b        = zeros<vec>(lag_b); // trailing past observations used in weighing and estimation for bursting regime
        vec H_r        = zeros<vec>(lag_b); // trailing past observations used in weighing and estimation for resting regime
...
// *********************************************    main loop over observations    *********************************************
    ...
    for(int n=1;n<N;n++){ // loop over N observations, omitting the first 'lag' observations and 'lag_s' (due to fixed lag smoothing)
// cout << "n=" << n << endl;
            if(n < s_lag_in){
              lag_s = n;
            } else{
...
// cout << "Step 1: sample particles" << endl;
                  if(x.n_elem!=N){
                    // sample particles
...
// cout << "Step 3: set intensities and weights" << endl;
                      Pn = P.col(n);      // set working parameters
                      Xn = X.col(n);      // set working covariance parameters


                      // set trailing history for bursting estimation
                      if(n < lag_b){ // if lag length is not yet met, use whatever is available
                        tmpvec = y.rows(0,n-1);
                        H_b    = rev_vec(tmpvec);
                      } else{
                        tmpvec = y.rows(n-lag_b,n-1);
                        H_b = rev_vec( tmpvec );
                      }
                      ...
                      if(pb>1){
                        ...
                        tmpvec = tmpmat.cols(0,H_b.n_elem-1)*H_b+Pn.col(0);
                      } else{
                      ...
                      // set trailing history for resting estimation
                      if(n < lag_r){ // if lag length is not yet met, use whatever is available
                        tmpvec = y.rows(0,n-1);
                        H_r = rev_vec( tmpvec);
                      } else{
                        tmpvec = y.rows(n-lag_r,n-1);
                        H_r = rev_vec( tmpvec );
                      }


                      if(pr>1){
                        // history dependency in the resting kernel, ie spline based regression, otherwise only baseline is included
                        tmpmat = Pn.cols(pb+1,pb+pr-1)*Sr.t();
                        tmpvec = tmpmat.cols(0,H_r.n_elem-1)*H_r+Pn.col(pb);
                      } else{
                        // only baseline firing is included when resting, hence no contribution other than baseline intensity
                        tmpvec = Pn.col(pb);
                      }
                      ...
                      if(any(Yn > 1/dt)){ // cap the intensity to avoid overflow
                        idx = find(Yn > 1/dt);
                        Yn(idx) = 1/dt*ones<vec>(idx.n_elem);
                      }
...
// cout << "Step 4: resampling" << endl;
                      ...
                      if(Neff < Neffpct*M){
                        ...
                        // Hence, resample up to lag_s lagged values, i.e. a fixed lag smoother.

                          idx   = resample(wn);        // resample indices based on weights
                          idx_h = regspace<uvec>(n-lag_s, n);

                          // tmpmat  = X.cols(n-lag_s,n);
                          // X.cols(n-lag_s,n) = tmpmat.rows(idx);
                          X.cols(idx_h) = X.submat(idx,idx_h);
                          Y.cols(idx_h) = Y.submat(idx,idx_h);

                          tmpcube = P.cols(n-lag_s,n);
                          for(int j=0; j<p; j++){
                            tmpmat           = tmpcube.slice(j);
                            tmpcube.slice(j) = tmpmat.rows(idx);
                          }
                          P.cols(n-lag_s,n) = tmpcube;

                          tmpcube = W.cols(n-lag_s,n);
                          for(int j=0; j<p2; j++){
                            tmpmat           = tmpcube.slice(j);
                            tmpcube.slice(j) = tmpmat.rows(idx);
                          }
                          W.cols(n-lag_s,n) = tmpcube;
...
// cout << "Step 5: i) evaluate particles" << endl;
                      ...
                      if( any(Xn>0) ){
                        ...
                        tmpvec = zeros<vec>(p);
                        tmpvec(0) = 1;
                        tmpmat2 = Sb.rows(0,H_b.n_elem-1);
                        tmpvec.subvec(1,pb-1) = tmpmat2.t()*H_b;
                      ...
                      if( any(Xn<=0) ){
                        ...
                        tmpvec = zeros<vec>(p);
                        tmpvec(pb) = 1;
                        tmpmat2 = Sr.rows(0,H_r.n_elem-1);
                        tmpvec.subvec(pb+1,pb+pr-1) = tmpmat2.t()*H_r;
                        tmpmat.each_row() = tmpvec.t();
                        dloglam.rows(idx) = tmpmat;
                      }
...
// cout << "Step 5: ii) update posteriors" << endl;
                      ...
                      for(int m=0;m<M;m++){
                            ...
                            tmpmat2   = .5*(tmpmat2+tmpmat2.t()); // ensure symmetry (flag this later)
                            tmpmat2   = inv(tmpmat2)+tmpmat;
                            tmpmat   = .5*(tmpmat2+tmpmat2.t()); // ensure symmetry (flag this later)

                            if(approx_equal(tmpmat, tmpmat.t(), "absdiff", 1e-6)){
                              tmpmat    = inv(tmpmat);
                            } else{
                              tmpmat = eye<mat>(p,p);
                            }
                    ...
                    if(verbose){ // display current progress
                      display_progress((double) n/N);
                    }
      } // end of observation loop


    // aggregate parameter estimates
    for(int n=0;n<N;n++){ // loop over N observations, omitting the first 'lag' observations and 'lag_s' (due to fixed lag smoothing)
